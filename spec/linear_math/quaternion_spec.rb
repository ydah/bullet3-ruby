...
RSpec.describe Bullet3::Quaternion do
  it "builds identity and array representations" do
    ...
    expect(quaternion.axis).to eq(Bullet3::Vector3.new(1, 0, 0))
    expect(quaternion.angle).to eq(0.0)
  end

  it "builds a quaternion from an axis and angle" do
    quaternion = described_class.from_axis_angle(Bullet3::Vector3.new(0, 0, 1), Math::PI)
  ...
  it "multiplies and inverts quaternions" do
    quaternion = described_class.from_axis_angle(Bullet3::Vector3.new(0, 1, 0), Math::PI / 4)
  ...
  it "spherically interpolates rotations" do
    ...
    finish = described_class.from_axis_angle(Bullet3::Vector3.new(0, 0, 1), Math::PI)

    halfway = start.slerp(finish, 0.5)

    expect(halfway.angle).to be_within(1e-6).of(Math::PI / 2)
    expect(halfway.axis.absolute).to eq(Bullet3::Vector3.new(0, 0, 1))
  end
end
