...
RSpec.describe "Bullet soft bodies" do
  before do
  ...
  it "creates helper soft bodies and steps a soft-rigid world" do
    ...
    anchor_body = rigid_body(Bullet::Vector3.new(0, 1, 0))
    world.add_rigid_body(anchor_body)

    rope = Bullet::SoftBody::Helpers.create_rope(info, [0, 1, 0], [1, 1, 0], 4, 1)
    rope.set_total_mass(1.0)
    rope.generate_bending_constraints(2)
    rope.wind_velocity = [0.1, 0, 0]
    rope.append_anchor(0, anchor_body)

    patch = Bullet::SoftBody::Helpers.create_patch(
      info,
      [-1, 1, -1],
      [1, 1, -1],
      [-1, 1, 1],
    ...
    ellipsoid = Bullet::SoftBody::Helpers.create_ellipsoid(info, [0, 2, 0], [0.5, 0.25, 0.5], 8)
    ...
    expect(info.water_normal).to eq(Bullet::Vector3.new(0, 1, 0))
    ...
    expect(rope.wind_velocity).to eq(Bullet::Vector3.new(0.1, 0, 0))
    expect(rope.node_position(0)).to be_a(Bullet::Vector3)
    expect(rope.aabb).to all(be_a(Bullet::Vector3))
