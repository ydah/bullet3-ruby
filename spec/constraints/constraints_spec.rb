...
RSpec.describe "Bullet3 constraints" do
  before do
    skip "native extension only" unless Bullet3.native?
  end

  def rigid_body(origin)
    shape = Bullet3::Shapes::SphereShape.new(0.5)
    transform = Bullet3::Transform.new(Bullet3::Quaternion.identity, origin)
    motion_state = Bullet3::MotionState.new(transform)
    info = Bullet3::RigidBodyConstructionInfo.new(1.0, motion_state, shape)
    Bullet3::RigidBody.new(info)
  end

  it "creates and updates point-to-point constraints" do
    body_a = rigid_body(Bullet3::Vector3.new(0, 0, 0))
    body_b = rigid_body(Bullet3::Vector3.new(1, 0, 0))
    constraint = Bullet3::Constraints::Point2PointConstraint.new(body_a, body_b, [0, 0, 0], [0, 0, 0])
    ...
    expect(constraint.pivot_in_a).to eq(Bullet3::Vector3.new(0.25, 0, 0))
  ...
  it "creates hinge and fixed constraints and adds them to worlds" do
    ...
    body_a = rigid_body(Bullet3::Vector3.new(0, 2, 0))
    body_b = rigid_body(Bullet3::Vector3.new(1, 2, 0))
    world.add_rigid_body(body_a)
    world.add_rigid_body(body_b)

    hinge = Bullet3::Constraints::HingeConstraint.new(
      body_a,
      body_b,
      [0, 0, 0],
      [0, 0, 0],
    ...
    fixed = Bullet3::Constraints::FixedConstraint.new(
      body_a,
      body_b,
      Bullet3::Transform.identity,
      Bullet3::Transform.identity
    )
  ...
  it "creates slider, cone twist, six-dof, gear, and hinge2 constraints" do
    world = Bullet3::DiscreteDynamicsWorld.create
    body_a = rigid_body(Bullet3::Vector3.new(0, 3, 0))
    body_b = rigid_body(Bullet3::Vector3.new(1, 3, 0))
    world.add_rigid_body(body_a)
    world.add_rigid_body(body_b)
    frame = Bullet3::Transform.identity

    slider = Bullet3::Constraints::SliderConstraint.new(body_a, body_b, frame, frame)
    ...
    cone_twist = Bullet3::Constraints::ConeTwistConstraint.new(body_a, body_b, frame, frame)
    ...
    cone_twist.motor_target = Bullet3::Quaternion.identity

    generic = Bullet3::Constraints::Generic6DofConstraint.new(body_a, body_b, frame, frame)
    ...
    spring2 = Bullet3::Constraints::Generic6DofSpring2Constraint.new(body_a, body_b, frame, frame, 0)
    ...
    gear = Bullet3::Constraints::GearConstraint.new(body_a, body_b, [0, 1, 0], [0, 1, 0], 2.0)
    gear.axis_a = [1, 0, 0]
    gear.axis_b = [0, 0, 1]
    gear.ratio = 3.0

    hinge2 = Bullet3::Constraints::Hinge2Constraint.new(body_a, body_b, [0, 3, 0], [0, 1, 0], [1, 0, 0])
    ...
    expect(generic.linear_lower_limit).to eq(Bullet3::Vector3.new(-0.25, -2, -3))
    expect(generic.angular_upper_limit).to eq(Bullet3::Vector3.new(0.1, 0.2, 0.3))
    ...
    expect(spring2.linear_upper_limit).to eq(Bullet3::Vector3.new(0.5, 0.5, 0.5))
    expect(spring2.limited?(3)).to be(true)

    expect(gear.constraint_type).to eq(:gear)
    expect(gear.axis_a).to eq(Bullet3::Vector3.new(1, 0, 0))
    expect(gear.axis_b).to eq(Bullet3::Vector3.new(0, 0, 1))
    expect(gear.ratio).to be_within(1e-6).of(3.0)

    expect(hinge2.constraint_type).to eq(:hinge2)
    expect(hinge2.axis1).to eq(Bullet3::Vector3.new(0, 1, 0))
    expect(hinge2.axis2).to eq(Bullet3::Vector3.new(1, 0, 0))
