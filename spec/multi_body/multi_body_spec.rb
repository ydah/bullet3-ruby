...
RSpec.describe "Bullet multi bodies" do
  before do
    skip "native extension only" unless ENV["BULLET_RUBY_USE_NATIVE"] == "1"
  end

  def rigid_body
    shape = Bullet::Shapes::SphereShape.new(0.25)
    motion_state = Bullet::MotionState.new(Bullet::Transform.identity)
    info = Bullet::RigidBodyConstructionInfo.new(0.0, motion_state, shape)
    Bullet::RigidBody.new(info)
  end

  it "creates multi bodies, colliders, and constraints" do
    world = Bullet::MultiBody::MultiBodyDynamicsWorld.create
    world.gravity = [0, -9.81, 0]

    multi_body = Bullet::MultiBody::MultiBody.new(1, 1.0, [1, 1, 1], false, true)
    ...
    base_collider = Bullet::MultiBody::MultiBodyLinkCollider.new(multi_body, -1)
    base_collider.collision_shape = Bullet::Shapes::BoxShape.new(Bullet::Vector3.new(0.5, 0.5, 0.5))
    base_collider.world_transform = Bullet::Transform.identity
    multi_body.set_base_collider(base_collider)

    motor = Bullet::MultiBody::MultiBodyJointMotor.new(multi_body, 0, 0.0, 10.0)
    motor.set_velocity_target(0.5)
    motor.set_position_target(0.1)
    motor.erp = 0.2
    motor.rhs_clamp = 1.0

    limit = Bullet::MultiBody::MultiBodyJointLimitConstraint.new(multi_body, 0, -0.5, 0.5)
    limit.lower_bound = -0.25
    limit.upper_bound = 0.25

    p2p = Bullet::MultiBody::MultiBodyPoint2Point.new(multi_body, 0, rigid_body, [0, 0, 0], [0, 0, 0])
    ...
    expect(world.gravity).to eq(Bullet::Vector3.new(0, -9.81, 0))
    ...
    expect(multi_body.base_position).to be_a(Bullet::Vector3)
    ...
    expect(p2p.pivot_in_b).to eq(Bullet::Vector3.new(0.1, 0, 0))
    expect(base_collider.link).to eq(-1)
    expect(base_collider.collision_shape).to be_a(Bullet::Shapes::CollisionShape)
